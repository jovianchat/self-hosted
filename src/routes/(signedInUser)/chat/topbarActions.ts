...
export const actions = {
    deleteChat: async (chatId: string) => {
        ...
        if (res.ok) {
            if (chatState.starred) {
                const chatIndex = sidebar.chatHistory.starred_history.findIndex((chat) => chat.id === chatId)
    ...
    renameChat: async (chatId: string, title: string) => {
        const access_token = await (await fetch('/hooks_fetchHandler')).text()
        const res = await fetch(`/http-server/chat/${chatId}/rename`, {
            method: 'PATCH',
            headers: {
    ...
    starChat: async (chatId: string) => {
        const access_token = await (await fetch('/hooks_fetchHandler')).text()
        const res = await fetch(`/http-server/chat/${chatId}/starred`, {
            method: 'PATCH',
            headers: {
                "Authorization": `Bearer ${access_token}`
            }
