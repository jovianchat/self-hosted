  ...
  {#if !sidebar.isOpen}
    <div class="tooltip tooltip-right mt-2" data-tip="Open Sidebar">
      <button class="btn btn-square btn-sm btn-ghost" onclick="{sidebar.toggle}">
        <TablerLayoutSidebarLeftExpandFilled class="w-9 h-9" />
      </button>
    </div>
    <a href="/" class="font-semibold text-xl">Jovian Chat</a>
  {/if}
</div>
<div class="absolute left-1/2 -translate-x-1/2 z-10 flex items-center gap-2">
  {#if chatId !== 'new' && chatId !== undefined}
    <GridiconsChat class="w-5 h-5" />
    {#if !isRenaming}
      <p>{chatState.title}</p>
    {:else}
      <form
...
<div class="flex gap-6">
  {#if chatId !== 'new' && chatId !== undefined}
    ...
    <div class="tooltip tooltip-bottom" data-tip="Delete Chat">
      <button
        ...
        class="btn btn-square btn-sm btn-ghost">
        <WeuiDeleteOnOutlined class="w-6 h-6" />
      </button>
    </div>
    <div class="tooltip tooltip-bottom" data-tip="Star Chat">
      <button
        ...
        {#if chatState.starred}
          <MdiStar class="w-6 h-6 text-yellow-500" />
        {:else}
  ...
  <div class="tooltip tooltip-left text-accent" data-tip="LLM Controls">
    <button
      type="button"
      class="btn btn-square btn-sm btn-ghost"
      onclick="{() => llmControlsModal!.showModal()}">
      <FluentSettingsChat16Filled class="w-6 h-6" />
    </button>
  </div>
</div>

<LlmControls bind:llmControlsModal={llmControlsModal!} {llmSavedSettings}/>
