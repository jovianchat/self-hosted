use crate::{models::llm::ApiConfig, Context, Result};
...
fn read_toml() -> Result<ApiConfigs> {
    let content = std::fs::read_to_string("admin/llm-api-config.toml")
...
fn save_toml(toml: ApiConfigs) -> Result<()> {
    ...
    std::fs::write("admin/llm-api-config.toml", content)
