use crate::{models::llm::PromptConfig, Context, Result};
...
fn read_toml() -> Result<Configs> {
    let content = std::fs::read_to_string("admin/prompt-engineering.toml")
...
fn save_toml(toml: Configs) -> Result<()> {
    ...
    std::fs::write("admin/prompt-engineering.toml", content)
