...
use crate::models::llm::{ApiConfig, LlmSdk, PromptConfig};
use crate::Result;
...
async fn generate_title(query: String) -> Result<String> {
    let llm_cfg = crate::routes::llm_settings::get_selected_fn()?;

    Ok(match llm_cfg.0.sdk {
...
async fn anthropic_title(
    (api_config, model, prompt_config): (ApiConfig, String, PromptConfig),
    query: String,
) -> Result<String> {
